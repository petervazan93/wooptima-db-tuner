...
- **Isolate daemon validation output from `mysql`.** Configuration validation uses a `root:mysql` mode `0710` parent and keeps probe and validation logs as root-owned mode `0600` files. Only its dedicated MariaDB validation datadir is owned by `mysql:mysql` and writable at mode `0700`.
