...
Backup handling is independent of force. An absent backup-evidence artifact or a valid artifact with `status=unknown` can continue only on a TTY after the separate exact active-language phrase `I CONFIRM A RESTORABLE BACKUP` or `POTVRDZUJEM OBNOVITELNU ZALOHU`; confirmed `missing`, malformed, stale, or future evidence blocks apply and cannot use that confirmation path.
