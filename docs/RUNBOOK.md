...
- Bezny `apply` ocakava stav `proposed`, aspon 288 validnych vzoriek a `proposal-manifest.tsv`, ktory SHA-256 hashmi viaze proposal na aktualny audit, samples a analysis.
...
- Apply historia sa nikdy neprepisuje. Obsahuje `manifest.tsv`, `proposed.cnf`, volitelny `original.cnf`, baseline, validacne/rollback artefakty a `ROLLBACK.txt`.
